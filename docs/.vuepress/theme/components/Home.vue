<template>
  <div class="home-container" :class="recoShow?'reco-show': 'reco-hide'">
    <div class="home-header">
      <div class="hero">
        <h1 v-if="data.isShowTitleInHome !== false">{{ data.heroText || $title || 'Clancy' }}</h1>

        <p class="description">{{ data.tagline || $description || 'Welcome to your blog' }}</p>
    ...
    <div class="home">
      ...
      <div class="footer">
        <span v-for="item in $themeConfig.footer || []">
          <i class="iconfont reco-github"></i>
          <a target="blank" :href="item.link">{{ item.text }}</a>
        </span>
        <!-- <span>
...
.home-container {
  ...
  .home {
    padding: $navbarHeight 2rem 8rem;
    max-width: 960px;
    margin: 0px auto;
    box-sizing: border-box;
